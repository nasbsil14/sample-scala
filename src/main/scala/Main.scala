...
object Main extends App {
  override def main(args: Array[String]): Unit = {
    ...
    if (1 < args.length) {
      args(0) match {
        case "imp" => new ImportProcessor(args(1)).exec()
        case "exp" => new ExportProcessor(args(1)).exec()
        case _ => println("methods invalid.")
      }
    } else {
      println("parameter shortage. p1: method p2: path")
    }
...
class ImportProcessor(filePath: String) {
  def exec(): Boolean = {
    ...
    if (Files.exists(path) && Files.isRegularFile(path)) {
      ...
      println("read file not exists:%s".format(path))
    }
    false
  }
...
class ExportProcessor(filePath: String) {
  def exec(): Boolean = {
    //OricoDetailRepository.getAll().foreach(println)
    OricoDetailRepository.getAll().onComplete{
      case Success(list) =>
